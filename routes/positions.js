...
const buildaccountbalancestablemiddleware = async function buildaccountbalancestable( req, res, next ) {
  ...
  for ( let index in bitmexaccounts ) {
    ...
    totalexposure += position.lastValue;
  ...
  let totalexposure = Number( totalxbtbalance * Number(usdperxbt) * 0.00000001 ).toFixed(2).toLocaleString();
  ...
  req.totalexposure = totalexposure; /* storing data to be used by the request handler in the Request.locals object */
