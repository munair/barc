...
const buildwalletinformationtablemiddleware = async function buildwalletinformationtable( req, res, next ) {
  ...
  let wallet = await restapirequest ( key, secret, 'GET', '/api/v1/user/wallet/' );
  // made requests.

  req.walletdata = wallet; /* storing data to be used by the request handler in the Request.locals object */
...
router.get( '/:accountid', buildwalletinformationtablemiddleware, (req, res, next)  => { res.render('walletinformation', { 'walletdata': req.walletdata, 'parameterdata': req.params } ); });
